...
if (conf.AUTO_LIKE_STATUS === "yes") {
    ...
    zk.ev.on("messages.upsert", async m => {
      ...
      var dev = ['254737681758', '254114141192'].map(t => t.replace(/[^0-9]/g) + "@s.whatsapp.net").includes(auteurMessage);
...
if (texte && texte.startsWith('>')) {
 ...
 //---------------------------------------rang-count--------------------------------
  ...
  try {
           ...
           // txt += `message supprimé \n @${auteurMessage.split("@")[0]} rétiré du groupe.`;
            ...
            var action = await atbrecupererActionJid(origineMessage);
              ...
              if (action === 'remove') {
                ...
                txt += `message deleted \n @${auteurMessage.split("@")[0]} removed from group by Beltah Hacking Team 👻.`;
    ...
    let warnlimit = conf.WARN_COUNT
 if ( warn >= warnlimit) { 
  var kikmsg = `XBOT detected a weak bot ;you will be remove because of reaching warn-limit`;
